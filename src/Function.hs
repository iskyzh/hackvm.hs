...
parseFunction args symbol =
    let (functionName, _args) = break (== ' ') args
        nVars                 = (read _args) :: Int
    in  [ Instruction $ "(" ++ functionName ++ ")"
...
parseReturn symbol =
    [ Instruction $ "@5"
        ...
        ++ [Instruction $ "@R13", Instruction $ "A=M", Instruction $ "0; JMP"]
