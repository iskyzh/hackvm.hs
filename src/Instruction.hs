...
parseInstruction instruction className symbol =
    let (cmd, args) = break (== ' ') instruction
    in  (Instruction $ "//" ++ instruction) -- add comments for current processing statement
    ...
    parseInstruction' cmd args = case cmd of
        ...
        "label"   -> [Instruction $ "(" ++ className ++ "." ++ args ++ ")"]
        "if-goto" -> popFromTop
            []
            [ Instruction $ "@" ++ className ++ "." ++ args
            , Instruction $ "D; JNE"
